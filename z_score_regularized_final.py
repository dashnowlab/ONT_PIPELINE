...
def zscore(v, mu, sd, period=None,
           lambda_sd=0.5,
           min_sd=0.5,
           min_repeat_diff=1.0):
    """
    Final adaptive regularized z-score.

    Regularization depends on:
      * minimum SD floor
      * observed SD
      * population median repeat count
    """
    v = safe_float(v)
    mu = safe_float(mu)
    sd = safe_float(sd)
    ...
    if abs(v - mu) < min_repeat_diff:
        return 0.0

    sd = max(sd, min_sd)

    # Scale regularization with the locus size (median repeat count)
    median_lambda = max(lambda_sd, 0.05 * abs(mu))

    # Also prevent unrealistically small SDs
    sd_lambda = 0.25 * sd

    adaptive_lambda = max(median_lambda, sd_lambda)

    effective_sd = math.sqrt(sd * sd + adaptive_lambda * adaptive_lambda)

    return (v - mu) / effective_sd


def norm_sf(z):
    """Two-sided p-value from regularized z-score."""
    z = safe_float(z)
    if math.isnan(z):
        return float("nan")
    z = abs(z)
...
def main():
    if len(sys.argv) != 4:
        sys.exit("Usage: python z_score_regularized_adaptive.py <input.vcf> <sample_name> <output.vcf>")
    ...
    new_headers = [
        ...
        '##INFO=<ID=TR_TENK10K_H1_P,Number=1,Type=Float,Description="Allele 1 two-sided p-value">',
        '##INFO=<ID=TR_TENK10K_H2_P,Number=1,Type=Float,Description="Allele 2 two-sided p-value">',
        ...
        '##INFO=<ID=TR_AOU1027_H1_P,Number=1,Type=Float,Description="Allele 1 two-sided p-value">',
        '##INFO=<ID=TR_AOU1027_H2_P,Number=1,Type=Float,Description="Allele 2 two-sided p-value">',
        ...
        '##INFO=<ID=TR_HPRC256_H1_P,Number=1,Type=Float,Description="Allele 1 two-sided p-value">',
        '##INFO=<ID=TR_HPRC256_H2_P,Number=1,Type=Float,Description="Allele 2 two-sided p-value">',
            ...
            if has_stranno:
                ...
                if motif_idx is not None:
                    ...
                    for pop, (med, sd) in stats.items():
                        z_values[f"{pop}_h1"] = zscore(a1_rep, med, sd, period)
                        z_values[f"{pop}_h2"] = zscore(a2_rep, med, sd, period)
